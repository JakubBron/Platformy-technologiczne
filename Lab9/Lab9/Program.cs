...
namespace Lab9
{
    class Program
    {
        ...
        {
            var projectedCars = myCars              // contains all "A6"-es and has "columns": engineType and hppl
            ...
            var groupped = projectedCars
                ...
                .OrderBy(t => t.Key);   // Key is engineType
        ...
        static void taskThree(string filename)
        {
            ...
            IEnumerable<XElement> models = rootNode
                ...
                //.XPathSelectElements("//car/engine[@model and not(@model = preceding::car/engine/@model)]");  -> unique engine types

            Console.WriteLine("Modele samochodów bez powtórzeń:");
            foreach(XElement e in models)
            {
                Console.WriteLine(e.Value);
                //Console.WriteLine(e.Attribute("model").Value);
            }
        }
